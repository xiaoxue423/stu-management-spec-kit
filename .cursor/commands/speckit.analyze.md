---
description: 在任务生成后, 对 spec.md、design.md 和 tasks.md 执行非破坏性的跨工件一致性和质量分析.
---
...
在实施之前, 识别三个核心工件(`spec.md`、`design.md`、`tasks.md`)之间的不一致、重复、模糊和规范不足的项目. 此命令**必须**在 `/speckit.tasks` 成功生成完整的 `tasks.md` 后运行.
...
- **严重**: 违反章程 MUST、缺失核心规范文档, 或零覆盖的需求阻止基线功能
