...
2. **加载设计文档**: 从 FEATURE_DIR 读取: 
   - **必需**: design.md(技术方案), spec.md(带优先级的用户故事)
   - 注意: 并非所有项目都有所有文档. 基于可用内容生成任务.

3. **执行任务生成流程**(遵循模板结构): 
   - 加载 design.md 并提取技术栈、库、项目结构
   - **加载 spec.md 并提取带优先级的用户故事(P1、P2、P3 等)**
   - 加载 design.md: 提取`第 5 章"核心实体"` → 映射到用户故事
   - 加载 design.md: 提取`第 7 章 API设计` → 映射API到用户故事
   - 加载 design.md: 提取`第 6 章 关键功能` → 生成设置任务
   - **按用户故事组织生成任务(重点)**: 
     - 设置任务(所有故事需要的共享基础设施)
