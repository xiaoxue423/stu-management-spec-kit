...
3. 加载和分析实施上下文: 
   ...
   - **必需**: 读取 design.md 获取实体和关系
   - **必需**: 读取 design.md 获取 API 规范和测试要求
   - **必需**: 读取 design.md 获取技术决策和约束
...
10. **Context沉淀**(在所有任务完成后):
- **检测项目环境**:
   - 检查是否存在`.mdp/workflows/spec-to-context.md`文件
   - 如果不存在，提示用户:"项目未配置spec-to-context workflow，跳过context沉淀"并结束此步骤
   - 如果存在，继续执行context沉淀

- **调用spec-to-context workflow**:
   - 传递参数: FEATURE_DIR的绝对路径
   - 说明: "正在将spec产物沉淀到项目context文档..."
   - 等待workflow执行完成
   - 展示workflow返回的合并报告

11. 在完成所有步骤并报告完成情况后, 从仓库根目录运行以下脚本标记阶段结束。脚本执行结果不影响流程:
    脚本参数：根据用户输入的ones任务解析出 ONES_TASK_ID，例如 https://ones.sankuai.com/ones/product/47458/workItem/task/detail/123，则ONES_TASK_ID=123
```bash
bash .specify/scripts/bash/notify-sdd-status.sh \
  --taskId {ONES_TASK_ID} \
...
- **完成确认**:
   - 向用户报告:"✅ Spec产物已成功沉淀到项目context文档"
   - 提供更新的context文档路径列表
   - 如果workflow执行失败，报告错误信息但不中断implement流程
