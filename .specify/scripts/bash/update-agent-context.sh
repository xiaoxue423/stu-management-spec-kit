...
DESIGN_FILE="$DESIGN"  # Path to design.md
AGENT_TYPE="${1:-}"
...
CATPAW_FILE="$REPO_ROOT/.catpaw/rules/specify-rules.md"
...
if [[ ! -f "$design_file" ]]; then
    ...
    NEW_LANG=$(extract_design_field "语言/版本" "$design_file")
    NEW_FRAMEWORK=$(extract_design_field "主要依赖" "$design_file")
    NEW_DB=$(extract_design_field "存储" "$design_file")
    NEW_PROJECT_TYPE=$(extract_design_field "项目类型" "$design_file")
...
update_specific_agent() {
    ...
    case "$agent_type" in
        ...
        q)
            ...
            ;;
        catpaw)
            update_agent_file "$CATPAW_FILE" "CatPaw"
            ;;
        *)
            log_error "Unknown agent type '$agent_type'"
            log_error "Expected: claude|gemini|copilot|cursor-agent|qwen|opencode|codex|windsurf|kilocode|auggie|roo|codebuddy|amp|q|catpaw"
            exit 1
            ;;
    esac
}
...
print_summary() {
    ...
    log_info "Usage: $0 [claude|gemini|copilot|cursor-agent|qwen|opencode|codex|windsurf|kilocode|auggie|roo|codebuddy|amp|q|catpaw]"
}
